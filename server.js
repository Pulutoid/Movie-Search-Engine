...
app.post('/signup', upload.single('picture'), async (req, res) => {
  try {
    ...
    // Set the profileID in a cookie
    res.cookie('profileID', profileID, { maxAge: 10 * 365 * 24 * 60 * 60 * 1000, httpOnly: true });

    // Redirect to the Get Started page after successful sign-up
  ...
  app.get('/viewProfile', async (req, res) => {
    try {
      ...
      // Render the viewProfile.html template, passing the profile data including the picture
  ...
  app.get('/switchProfile', async (req, res) => {
    try {
      ...
      // Render the switchProfile.html template, passing the list of profiles including their pictures
